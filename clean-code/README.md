...
6. Не оставлять закомментированный код. Такой код никак не поддерживается и только мешается при чтении программы.
  Система контроля версий сохранит этот код на случай, если к нему придется вернуться.
...
1. Ради тестов можно ослаблять инкапсуляцию класса, но только если других вариантов написания теста нет.

2. Классы должны быть компактными с точки зрения ответственности. Классу с маленькой ответственностью можно легко
  подобрать четкое короткое имя. Система должна состоять из множества мелких классов, а не из небольшого числа больших.

3. Соблюдать принцип единственной ответственности. У класса должна быть только одна причина для изменения.

4. Класс должен обладать высокой связностью. Чем с большим количеством переменных класса взаимодействует метод,
  тем выше его связность с классом. Если связность класса маленькая, то из него можно выделить хотя бы один класс.

5. Соблюдать принцип открытости/закрытости. Класс должен быть открыт для расширения, но закрыт для модификации.

6. Соблюдать принцип обращения зависимостей. Класс должен зависеть не от конкретной зависимости, а от интерфейса.

## Глава 11. Системы

1. Отделять конструирование системы от ее использования. Отложенная инициализация - нарушение этого принципа.
  Отложенную инициализацию сложно тестировать, а для ее работы в рантайме требуются все зависимости, даже если на деле
  она не будет осуществлена.

2. Внедрение зависимостей - мощный механизм разделения инициализации и использования.

## Глава 12. Формирование архитектуры

1. **Правило простой архитектуры №1: выполнение всех тестов.** Тесты обеспечивают контролируемость системы.
  Существование тестов приводит к написанию более удобных для тестирования, а значит и более правильных, классов.
  Написание тестов улучшает архитектуру системы.

2. **Правило простой архитектуры №2: отсутствие дублирования.** Дублирование может заключаться не только в совпадении
  строк кода, но и в реализации одной и той же логики двумя разными способами. Такие места в системе необходимо находить
  и рефакторить.

3. **Правило простой архитектуры №3: выразительность.** Чем понятнее будет код, тем меньше времени понадобится
  программисту, чтобы разобраться в нем. Для выразительности нужно тщательно думать над каждой функцией и классом
  и соблюдать правила написания чистого кода.

3. **Правило простой архитектуры №4: использование минимума классов и методов.** Это правило обладает наименьшим
  приоритетом по сравнению с тремя предыдущими, однако про него стоит помнить и не доводить разделение системы
  на мельчайшие классы до фанатизма, чтобы сама система не разрасталась до огромных размеров.

## Глава 13. Многопоточность

1. Отделять код, относящийся к реализации многопоточности, от остального кода.

2. Максимально инкапсулировать данные. Ограничить доступ ко всем общим данным.

3. Копировать данные, чтобы у каждого потока был свой экземпляр данных. Такой подход _может_ быть быстрее,
  чем использование защитных блокировок.

4. Максимально разбить данные на независимые подмножества, с которыми могут работать независимые потоки. В таком случае
  нет необходимости синхронизации данных между потоками.

5. Не игнорировать ошибки, считая их случайными разовыми сбоями. В многопоточном коде ошибки могут возникать
  очень редко, но это признак того, что в коде что-то не так.

6. Реализовать многопоточный код так, чтобы его выполнение можно было конфигурировать. Например, настраивать количество
  потоков, скорость выполнения тестовых заменителей, динамически менять (во время выполнения) количество потоков.

7. Тестировать многопоточную программу с количеством потоков, превышающих число процессоров или ядер. Это увеличивает
  вероятность появления ошибок при переключении программы между потоками.

8. Многопоточный код необходимо тестировать во всех возможных средах (платформах, операционных системах).
  Различные среды имеют разные политики управления потоками.
